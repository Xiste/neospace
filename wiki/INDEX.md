# 📚 Wiki Neospace — Índice de Conhecimento

## Progresso Geral
- Tópicos estudados: 13 de ~25
- 🔴 Não estudado | 🟡 Em estudo | 🟢 Dominado

---

## Fundamentos do Transformer (Attention Is All You Need)

- [x] [Tokenização (BPE)](TOKENIZACAO.md) 🟢
- [x] [Embeddings](EMBEDDINGS.md) 🟢
- [x] [Self-Attention](SELF_ATTENTION.md) 🟢
- [x] [Multi-Head Attention](MULTI_HEAD_ATTENTION.md) 🟢
- [x] [Atenção no Codificador/Decodificador](ATENCAO_CODIFICADOR_DECODIFICADOR.md) 🟢
- [x] [Positional Encoding](POSITIONAL_ENCODING.md) 🟢
- [x] [Feed-Forward + Residual + LayerNorm](FEED_FORWARD_RESIDUAL_LAYERNORM.md) 🟢
- [x] [Transformer — Arquitetura Completa](TRANSFORMER_COMPLETO.md) 🟢
- [x] [Pipeline Completa — Exemplo Passo a Passo](PIPELINE_COMPLETA.md) 🟢
- [x] [Transformer do Zero em PyTorch](TRANSFORMER_PYTORCH.md) 🟢

## Técnicas de Eficiência de Atenção
- [x] [Flash Attention](FLASH_ATTENTION.md) 🟢
- [x] [Multi-Query & Grouped Query Attention](MQA_GQA.md) 🟢
- [x] [Grouped Query Attention](MQA_GQA.md) 🟢

## Técnicas de Paralelismo
- [ ] Data Parallel (DDP)
- [ ] Fully Sharded Data Parallel (FSDP)
- [ ] Pipeline Parallelism
- [ ] Tensor Parallelism

## Técnicas de Fine-tuning Eficiente
- [ ] LoRA
- [ ] QLoRA
- [ ] Prefix Tuning

## Quantização
- [ ] INT8
- [ ] INT4
- [ ] GPTQ
- [ ] AWQ

## Arquiteturas Modernas
- [ ] Mixture of Experts (MoE)
- [ ] State Space Models (Mamba)
- [ ] Sparse Attention

## Glossário
- [x] [Glossário de Termos](GLOSSARIO.md)

## Papers Importantes
- [x] [Attention is All You Need (2017)](TRANSFORMER_COMPLETO.md) 🟢
- [x] [Flash Attention](FLASH_ATTENTION.md) (2022) 🟢
- [ ] LoRA (2021)
- [ ] DeepSeek V4 (2026)

---

## Próximos na Trilha (Sugestões)
1. Multi-Query / Grouped Query Attention — evolução natural após entender multi-head
2. LoRA — fine-tuning eficiente (pré-requisito: transformer completo)
3. Paralelismo (DDP, FSDP) — como treinar modelos grandes
